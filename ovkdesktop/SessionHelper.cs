...
namespace ovkdesktop
{
    public static class SessionHelper
    {
        ...
        public static async Task<OVKDataBody> LoadTokenAsync()
        {
            try
            {
                string filePath = Path.Combine(App.LocalFolderPath, "ovkdata.json");
                ...
                {
                    var tokenData = await JsonSerializer.DeserializeAsync<OVKDataBody>(fs);
                    return tokenData;
                }
            }
            catch (Exception ex)
            {
                Debug.WriteLine($"[SessionHelper] Ошибка загрузки OVKDataBody: {ex.Message}");
                return null;
            }
        }
